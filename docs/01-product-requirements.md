...
| D-8 | How Inventory learns stock came back from a return | Retail Sales publishes `ItemReturned` when a return is processed; Inventory consumes it and adds the returned quantity straight back onto On Hand at the location the sale was fulfilled from, applied once per (returnId, product, location) | gRPC's `ReleaseReservation` only releases a pending (not-yet-sold) reservation, not a completed sale, so a return needed its own mechanism. A new RPC would touch a stable, cross-team-owned contract for a case that isn't the latency-sensitive live checkout gRPC is reserved for; reusing the manual stock-adjustment REST endpoint has no idempotency key. An event matches the bus pattern used everywhere else in the system | FR-6.4 |
| D-9 | Sales Audit's expected-total mechanism | Sales Audit keeps a running expected-total accumulator by consuming `ItemSold` continuously; the REST call to Retail Sales for a store's expected total is a point-in-time cross-check at close time, not the primary mechanism — if the two disagree, the REST answer wins for that close, and the mismatch is only logged | FR-7.7 mandates consuming the event stream directly; the REST call in §5 exists so a dropped/late `ItemSold` message can never silently misstate a store's close, mirroring how Receiving backfills from Procurement when an event hasn't arrived yet | FR-7.1, FR-7.7, §5 |
